...
\hypersetup{
  ...
  citecolor=ispgreen,
  urlcolor=ispblue,
  pdftitle={Educational Manual for ISP v2.4},
...
\newcommand{\app}{\textsf{ISP v2.4}\index{ISP v2.4}}
...
\begin{titlepage}
  ...
  \vspace{1cm}
  {\large Manual version 2.4 \quad Date: 2026-05-30\par}
  \vfill
...
\section{The five research paths}
...
\field{one_group_post_survey} & The same post-intervention survey is answered once by one group. & Describing opinions, perceived learning, usability, or experience with confidence intervals, without making a causal effect claim.\\
\field{stratified_post_survey} & The same post-intervention survey is answered once, but valid responses are planned and checked inside demographic strata. & Describing opinions while checking that age bands, regions, school types, experience levels, or other planned classes are represented.\\
\bottomrule
\end{longtable}
...
Binary outcomes are supported for the two independent group path. For pre-test/post-test paths, the app currently supports continuous outcomes. For post-intervention surveys, including stratified surveys, the app treats the outcome as an ordered or numeric survey response and plans precision for either a favorable-response proportion or a mean score.
...
This path applies when participants are distinct across groups and the main comparison is between groups. It fits post-test-only studies, survey scores compared between intervention and control, and binary success-rate comparisons.
...
\section{Path 4: One-group post-intervention survey}

Choose \field{one_group_post_survey} when the only measurement is an opinion, usability, experience, or perceived-learning survey answered after the intervention. This path is useful for MEEGA+-style educational-game questionnaires, Likert agreement items, star ratings, and bounded numeric scores \parencite{petri2019meega}.

\begin{examplebox}{Uno post-use questionnaire}
After a guided Uno activity, students answer items such as "The game was easy to learn" and "The game helped me understand greater-than and less-than." The response scale is 1 to 5, where 4 and 5 count as favorable. The researcher wants to say how precisely the sample supports a statement about favorable opinions.
\end{examplebox}

\subsection*{When it applies}

This path applies when the research question is descriptive: what users reported after using the intervention. A typical claim is:

\begin{quote}
With 95\% confidence, the favorable-response proportion is approximately between 68\% and 86\%.
\end{quote}

The app can plan the number of valid respondents needed for a desired margin of error, and it can evaluate a completed histogram of responses.

\subsection*{What it cannot cover well}

This path does not estimate causal learning gain. It has no pre-test, no control group, and no counterfactual comparison. If the researcher wants to show that the game improved learning, use \field{pretest_posttest_control} or another controlled design. If the researcher wants to validate a questionnaire psychometrically, use reliability, factor analysis, and measurement-validation methods beyond this app.

\section{Path 5: Stratified post-intervention survey}

Choose \field{stratified_post_survey} when the survey is still answered after the intervention only, but the researcher wants the result to represent planned demographic classes. A \term{stratum}\index{stratum} is a class such as age band, region, school type, prior experience, gender, or another demographic grouping defined before recruitment.

\begin{examplebox}{Uno post-use questionnaire with age-band representation}
After the same guided Uno activity, the researcher wants the opinion survey to represent three age bands: 8--10, 11--13, and 14--16. The intended population is about 30\%, 40\%, and 30\% in those bands. If older children answer faster, a simple overall survey could overstate their opinions. The stratified path plans valid responses inside each band and later checks whether any band was under-represented.
\end{examplebox}

\subsection*{When it applies}

This path applies when the claim is descriptive and opinion-based, but the intended audience includes identifiable subgroups. It is useful when the researcher wants to say something like:

\begin{quote}
The opinion result represents the intended age bands reasonably well, and not only the easiest group to recruit.
\end{quote}

The main inputs are \field{strata_definition}, \field{stratified_allocation_method}, \field{stratified_min_per_stratum}, \field{stratified_target_total}, \field{stratified_population_known}, and \field{stratified_use_weights}. The \field{strata_definition} field is JSON. A minimal example is:
...
The app can allocate proportionally, equally, by a minimum per stratum, or manually using \field{target_valid_n} inside each stratum.

\subsection*{What it cannot cover well}

The stratified survey path does not make a causal claim. It also does not replace a full survey-sampling analysis for complex multistage designs, non-response modeling, calibration weighting, or post-stratification. If strata are crossed too aggressively, for example age by gender by region by school type, cells can become too small for stable interpretation.
...
\textbf{Why the wizard asks:} the calculation changes depending on whether the comparison is between different people, between two groups over time, within the same people over time, or a descriptive post-intervention survey.
...
\field{one_group_post_survey} & One group answers a post-use opinion survey. & The claim is about causal learning gain or change from baseline.\\
\bottomrule
\end{longtable}
...
\midrule
\endhead
\field{continuous} & Test score, time, scale score, number correct, confidence rating. & Supported for all comparison paths and used internally for survey scales.\\
\field{binary} & Passed/not passed, completed/not completed, success/failure. & Supported for two independent groups.\\
\bottomrule
\end{longtable}

\section{Step 3A: Survey target, scale, and favorable response}

This step appears when \field{study_design} is \field{one_group_post_survey}.

\begin{decisionbox}{Question}
What kind of survey statement do you want to support?
\end{decisionbox}

\textbf{Configuration variables:} \field{survey_analysis_goal}, \field{survey_scale_min}, \field{survey_scale_max}, \field{survey_scale_points}, \field{survey_favorable_threshold}, \field{survey_target_proportion}, \field{survey_expected_proportion}, \field{survey_margin_of_error}, \field{survey_expected_sd}, \field{survey_mean_margin_of_error}, and \field{survey_target_mean}.

\textbf{Why the wizard asks:} a post-intervention survey is usually an estimation problem. The app needs to know whether the main statement is about the percentage of favorable answers or the mean score, and it needs the scale definition so it can interpret a histogram.

\begin{longtable}{L{0.26\textwidth}L{0.28\textwidth}L{0.36\textwidth}}
\toprule
\textbf{Variable} & \textbf{Example value} & \textbf{How it is used}\\
\midrule
\endfirsthead
\toprule
\textbf{Variable} & \textbf{Example value} & \textbf{How it is used}\\
\midrule
\endhead
\field{survey_analysis_goal} & \field{favorable_proportion} & Plans either a proportion confidence interval or a mean-score confidence interval.\\
\field{survey_scale_min} & 1 & Lowest valid numeric response.\\
\field{survey_scale_max} & 5 & Highest valid numeric response.\\
\field{survey_scale_points} & 5 & Documents the number of ordered response options, excluding NA.\\
\field{survey_favorable_threshold} & 4 & Scores at or above this value count as favorable.\\
\field{survey_expected_proportion} & 0.50 & Planning value for the favorable proportion; 0.50 is conservative when uncertain.\\
\field{survey_margin_of_error} & 0.10 & Desired half-width of the confidence interval, in proportion units.\\
\field{survey_expected_sd} & 1.0 & Expected standard deviation for mean-score planning; can come from a pilot.\\
\field{survey_mean_margin_of_error} & 0.20 & Desired half-width of the mean-score confidence interval.\\
\bottomrule
\end{longtable}

\begin{examplebox}{MEEGA+-style Likert item}
For a 1--5 agreement item, a researcher may define 4 and 5 as favorable, set \field{survey_favorable_threshold} to 4, and plan for \field{survey_margin_of_error} of 0.10. This means the app estimates how many valid respondents are needed for roughly plus or minus 10 percentage points around the favorable-response proportion.
\end{examplebox}
...
For two-group designs, the wizard can ask for the group sizes:
...
If the researcher knows only the total achieved sample, the user may instead enter \field{observed_total_n}. In that case the answer is not unique because the allocation between control and intervention is unknown. The app therefore reports capacity rows for common allocations such as 1:1, 2:1, and 1:2.
...
\textbf{Why the wizard asks:} achieved power and approximate p-value depend on the information actually collected. In repeated-measure designs, the observed total should count only people with the required complete data. If no effect is entered, the app uses the sample size in the reverse direction: it estimates what effect sizes the achieved sample could detect under common \(p\)-value and power standards.
...
This field may be left empty when the researcher wants a sample-capacity analysis rather than an achieved-effect analysis. With only the sample size, the app cannot produce one unique \(p\)-value or achieved power. Instead, it reports a table of reverse calculations: minimum detectable effect for common alpha and power targets, approximate power for common effects, and approximate alpha thresholds for common effect/power targets.
...
\begin{itemize}
  ...
  \item If only sample size is known, what minimum detectable effect corresponds to common standards such as \(p<0.05\) with 80\% power?
  \item If only total two-group sample size is known, how do common allocations change the capacity of the sample?
...
\field{study_design} & All modes & Research path. & \code{parallel_two_group}, \code{pretest_posttest_control}, \code{one_group_pre_post}, \code{one_group_post_survey}.\\
...
\field{survey_analysis_goal} & Survey planning and evaluation & Whether the main survey claim uses favorable proportion or mean score. & \code{favorable_proportion} or \code{mean_score}.\\
\field{survey_scale_min} & Survey planning and evaluation & Lowest numeric survey response. & Often 0 or 1.\\
\field{survey_scale_max} & Survey planning and evaluation & Highest numeric survey response. & Often 5, 10, or 1 for normalized scales.\\
\field{survey_scale_points} & Survey documentation & Number of ordered valid response points, excluding NA. & 3 to 11; often 5 or 6.\\
\field{survey_favorable_threshold} & Survey evaluation & First response value counted as favorable. & 4 on a 1--5 agreement scale.\\
\field{survey_target_proportion} & Survey evaluation & Desired minimum favorable share for interpretation. & 0.60, 0.70, or 0.80.\\
\field{survey_expected_proportion} & Survey planning & Expected favorable share for precision planning. & 0.50 if uncertain; 0.70 or 0.80 with prior evidence.\\
\field{survey_margin_of_error} & Survey proportion planning & Desired confidence-interval half-width. & 0.05 to 0.10 is common.\\
\field{survey_expected_sd} & Survey mean planning & Expected standard deviation of survey score. & Pilot value, or blank for rough scale-based default.\\
\field{survey_mean_margin_of_error} & Survey mean planning & Desired mean confidence-interval half-width. & 0.20 to 0.50 scale points.\\
\field{survey_target_mean} & Survey evaluation & Mean score target for interpretation. & 4.0 on a 1--5 agreement scale.\\
...
\field{observed_control_n} & Evaluation mode & Achieved control sample. & Integer at least 2 for two-group evaluation; optional when total-only capacity analysis is used.\\
\field{observed_intervention_n} & Evaluation mode & Achieved intervention sample. & Integer at least 2 for two-group evaluation; optional when total-only capacity analysis is used.\\
\field{observed_total_n} & Evaluation mode & Achieved complete pairs or total two-group sample. & Integer at least 2 for one-group studies; at least 4 for two-group total-only capacity scenarios.\\
...
\field{observed_effect_size} & Evaluation mode & Effect actually observed. & Standardized effect or binary proportion difference; may be blank for sample-capacity analysis.\\
\field{observed_survey_counts} & Survey evaluation & JSON histogram of response counts. & Example: \code{\{"1":2,"2":4,"3":10,"4":18,"5":26,"NA":3\}}.\\
\field{observed_survey_favorable_count} & Survey evaluation & Favorable valid responses when no full histogram is entered. & Count from 0 to \field{observed_total_n}.\\
\field{observed_survey_mean} & Survey evaluation & Observed mean survey score when no histogram is entered. & Within the survey scale.\\
\field{observed_survey_sd} & Survey evaluation & Observed survey score standard deviation. & Positive number.\\
...
\field{survey_scale_min} & 0--10 & 0, 1 & Most opinion scales use small bounded values.\\
\field{survey_scale_max} & 1--10 & 5, 10, 1 & The maximum anchors thresholds and mean-score planning defaults.\\
\field{survey_scale_points} & 3--11 & 5, 6, 10 & Most Likert, star, and numeric rating scales fall in this range.\\
\field{survey_target_proportion} & 0.50--0.90 & 0.60, 0.70, 0.80 & Positive opinion claims below 50\% are usually weak; above 90\% is strict.\\
\field{survey_expected_proportion} & 0.30--0.90 & 0.50, 0.70, 0.80 & 0.50 is conservative for margin-of-error planning.\\
\field{survey_margin_of_error} & 0.03--0.15 & 0.05, 0.10 & Opinion surveys commonly use 5 to 10 percentage points.\\
\field{survey_expected_sd} & 0.20--3.00 & 0.8--1.2 on a 1--5 scale & Mean precision depends directly on the score SD.\\
\field{survey_mean_margin_of_error} & 0.05--1.00 & 0.20, 0.30, 0.50 & The margin should be meaningful on the response scale.\\
...
Some studies collect only a post-use opinion or usability scale. If there is a control group, use the two independent group path and compare groups. If there is no control group, use \field{one_group_post_survey}. In that path, the app does not ask for \field{effect_size_d}; it asks for the desired precision of a descriptive survey statement.

\begin{examplebox}{MEEGA+-style agreement item}
Suppose a game questionnaire uses a 1--5 agreement scale. Scores 4 and 5 mean the respondent agreed or strongly agreed. A practical survey claim might be: "at least 70\% of valid respondents were favorable." The relevant planning variables are the confidence level, \field{survey_expected_proportion}, and \field{survey_margin_of_error}; the result is a number of valid survey respondents, not a p-value.
\end{examplebox}

\begin{warningbox}{Survey evidence is not causal evidence}
A post-intervention survey can say that participants reported positive experience, perceived learning, or usability. It cannot by itself show that learning improved because there is no baseline and no comparison group.
\end{warningbox}
...
\section{One-group post-intervention survey}

For a favorable-response proportion, the app plans a descriptive confidence interval:

\[
n = \frac{z_{\alpha}^{2}p(1-p)}{e^{2}},
\]

where \(p\) is \field{survey_expected_proportion} and \(e\) is \field{survey_margin_of_error}. The value \(p=0.50\) is conservative because it produces the largest sample for a fixed margin of error.

For a mean survey score, the app uses:

\[
n = \left(\frac{z_{\alpha}s}{e_{\bar{x}}}\right)^2,
\]

where \(s\) is \field{survey_expected_sd} and \(e_{\bar{x}}\) is \field{survey_mean_margin_of_error}. If no expected standard deviation is entered, the app uses one quarter of the scale range as a rough planning value.

In completed-survey evaluation, numeric categories in \field{observed_survey_counts} are treated as valid response options. Labels such as \code{NA}, \code{N/A}, or other nonnumeric labels are reported separately and excluded from the valid opinion denominator. The app reports the favorable proportion and a Wilson confidence interval \parencite{wilson1927probable}. If a histogram or mean/standard deviation is available, it also reports a mean-score confidence interval.

\begin{statbox}{Why no p-value or power appears here}
The post-intervention survey path estimates a proportion or mean from one sample. Without a pre-intervention measure, control group, known benchmark, or planned hypothesis test, p-value and power would invite an overclaim. The key result is the confidence interval, especially its lower bound for favorable-response claims.
\end{statbox}
...
\begin{itemize}
  ...
  \item when no observed effect is entered, what the achieved sample size alone can support under common effect, alpha, power, and allocation assumptions.
\end{itemize}

When an observed effect is available, this table is not a promise that adding exactly that number of participants would reproduce the same effect. It is a diagnostic table: it shows what the current achieved result implies if the observed effect size is treated as the working estimate.

When only sample size is entered, the table becomes a sample-capacity table. For example, a one-group pre/post pilot with 40 complete pairs may show that \(p<0.05\) with 80\% power requires about \(d=0.44\), while \(d=0.50\) would have about 89\% power. This does not say that the study found \(d=0.44\); it says that effects smaller than that would be hard for that sample to detect under the selected assumptions.

For \field{one_group_post_survey}, the table reports descriptive survey evidence instead of p-value or power benchmarks. With only a valid respondent count, it shows the approximate current margin of error. With a response histogram, it shows valid responses, missing or NA responses, favorable responses, favorable proportion, confidence interval, and the mean score when it can be computed.
...
\begin{itemize}
  ...
  \item clustering;
  \item post-intervention survey designs where the evidence is descriptive rather than causal;
...
\section{Example 5: Post-intervention survey after Uno}

\subsection*{Scenario}

After a guided Uno activity, the researcher wants to evaluate player experience and perceived learning with a MEEGA+-style questionnaire. One item is: "The game helped me understand greater-than and less-than." The response scale is 1 to 5, with 1 = strongly disagree, 3 = neutral, 4 = agree, and 5 = strongly agree. Scores 4 and 5 are considered favorable.

The researcher wants a descriptive claim, not a causal claim: how precisely can the study describe students' favorable opinions after the activity? Because there is no prior pilot, the researcher uses \field{survey_expected_proportion}=0.50 for conservative planning and accepts a margin of error of 0.10.

\subsection*{Wizard choices}

\begin{longtable}{L{0.30\textwidth}L{0.28\textwidth}L{0.32\textwidth}}
\toprule
\textbf{Wizard field} & \textbf{Value} & \textbf{Reason}\\
\midrule
\endfirsthead
...
\midrule
\endhead
\field{study_design} & \field{one_group_post_survey} & The only measurement is a post-use questionnaire.\\
\field{workflow_path} & \field{plan_study} & The survey has not been collected.\\
\field{survey_analysis_goal} & \field{favorable_proportion} & The main claim is about the share of favorable responses.\\
\field{alpha} & 0.05 & This corresponds to a 95\% confidence interval.\\
\field{survey_scale_min} & 1 & Lowest valid response.\\
\field{survey_scale_max} & 5 & Highest valid response.\\
\field{survey_scale_points} & 5 & Five ordered Likert response options.\\
\field{survey_favorable_threshold} & 4 & Agree and strongly agree are favorable.\\
\field{survey_expected_proportion} & 0.50 & Conservative when no pilot exists.\\
\field{survey_margin_of_error} & 0.10 & About plus or minus 10 percentage points.\\
\field{completion_rate} & 0.90 & Some invited students may not answer all items.\\
\field{usable_data_rate} & 0.95 & Some answers may be invalid or unusable.\\
\bottomrule
\end{longtable}

\subsection*{Completed-survey evaluation}

After collection, suppose the item produces this histogram:

\begin{verbatim}
{"1": 2, "2": 4, "3": 10, "4": 18, "5": 26, "NA": 3}
\end{verbatim}

The app treats 60 numeric responses as valid, excludes the 3 NA responses from the valid opinion denominator, counts 44 favorable responses, and reports the favorable proportion with a Wilson confidence interval. The conservative interpretation should use the lower confidence bound: if the lower bound is below the desired target, the study may still be positive descriptively, but it does not justify the stronger statement that the target was reached with high confidence.

\section{Example 6: Inverse problem after a pilot}
...
\section{Example 7: Completed study compared with a previous plan}
